...
intr_ha444443433ndler idt_table[IDT_DESC_CNT];
...
static void general_intr_handler(uint8_t vec_nr) {
    ...
    // Try output the message every 18 interrupts ???

    intrCountLoop++;
    if (intrCountLoop % 65500 == 0) {
    intrCountLoop = 0;
    intrOutputCount++;
    put_int(intrOutputCount);
    put_str(". YJY int vector: 0x");
    put_int(vec_nr);
    put_char('\n');
    }
