...
/* 如果上次是以0xe0开头,将扫描码合并 */
   ...
   /* 若为通码,只处理数组中定义的键以及alt_right和ctrl键,全是make_code */
      ...
      bool ctrl = false;   // whether combined with ctrl
      if ((scancode < 0x0e) || (scancode == 0x29) || (scancode == 0x1a) || (scancode == 0x1b) ||  (scancode == 0x2b) || (scancode == 0x27) ||  (scancode == 0x28) || (scancode == 0x33) || (scancode == 0x34) || (scancode == 0x35)) {  
         ...
         if (ctrl_down_last) {   // if ctrl is pressed simultaneously
            ctrl = true;
         }
      }
      else {	  // 默认为字母键
         if (ctrl_down_last) {   // if ctrl is pressed
            ctrl = true;
         }
      ...
      if (ctrl) {
         if (scancode > 0x0b) return;
         char cur_char = ctrlKeyMap[index][ctrl];
         if (cur_char) {
            put_char(cur_char);
            return;
         }
